...
#'
#' @export
draw2d <- function(geodf,outfile,maptitle,mapWindow,
                      maptype="osm",minTiles=50,
                      mapsize=c(1600,1200),fine.map=FALSE,
                      draw.speed=FALSE,
  ...
  map.lon.min.dd <- mapWindow[1]
  map.lon.max.dd <- mapWindow[2]
  map.lat.min.dd <- mapWindow[3]
  map.lat.max.dd <- mapWindow[4]

  if (min(map.lat.max.dd-map.lat.min.dd,map.lon.max.dd-map.lon.min.dd) > .005){
